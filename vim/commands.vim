...
command! OpenUrl call s:OpenUrlUnderCursor()


" display highlight info of item below cursor, using SyntaxAttr plugin
command! SyntaxAttr call SyntaxAttr()


" toggle comment highlighting
function! s:ToggleCommentHl() abort
  " Redirect output of :highlight to l:current
  redir => l:current
  silent highlight Comment
  redir END

  if l:current =~# 'ctermfg=245'
    execute "colorscheme " . g:colors_name
  else
    highlight Comment ctermfg=245
  endif
endfunction
command! ToggleCommentHl call s:ToggleCommentHl()


" toggle hlsearch
command! ToggleHlSearch set hls! | set hls?

" clear last search pattern + highlighting
command! ClearSearch let @/=""


" toggle comment -- current line, no selection -- using ToggleComment plugin
command! ToggleComment call ToggleComment()

" toggle comments -- a visual selection
function! s:ToggleComments() abort
...
command! ToggleComments call s:ToggleComments()
